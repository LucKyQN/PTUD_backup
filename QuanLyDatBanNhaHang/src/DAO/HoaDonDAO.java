...
public class HoaDonDAO {
	...
	public List<Model.MonAnModel> getMonAnTheoBan(String maBan, String trangThaiBan) {
		...
		// TRƯỜNG HỢP 2: Bàn đã đặt (Màu vàng) -> Lấy từ Chi tiết Phiếu đặt
	...
	public boolean copyMonAnTuPhieuSangHoaDon(String maPhieu, String maHD) {
		// Câu lệnh SQL: Lấy dữ liệu từ ChiTietPhieuDat và INSERT vào ChiTietHoaDon
		// Lưu ý: donGia lấy từ bảng MonAn để đảm bảo giá mới nhất
		String sql = "INSERT INTO ChiTietHoaDon (maHD, maMonAn, soLuong, donGia, thanhTien, trangThaiPhucVu) "
				+ "SELECT ?, ct.maMonAn, ct.soLuong, m.giaBan, (ct.soLuong * m.giaBan), N'Chưa lên' "
				+ "FROM ChiTietPhieuDat ct " + "JOIN MonAn m ON ct.maMonAn = m.maMonAn " + "WHERE ct.maPhieu = ?";
		try {
			java.sql.Connection con = connectDatabase.ConnectDB.getInstance().getConnection();
			java.sql.PreparedStatement ps = con.prepareStatement(sql);
			ps.setString(1, maHD);
