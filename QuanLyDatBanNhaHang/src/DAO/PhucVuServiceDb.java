...
//			// Lấy từ Hóa đơn (Giữ nguyên)
//			sql = "SELECT m.maMonAn, m.tenMonAn, c.soLuong, m.giaBan, (c.soLuong * m.giaBan) as thanhTien, c.trangThaiPhucVu, c.id_cthd "
...
//		} else {
//			// SỬA THEO ẢNH: maDon, maMonAn, soLuonq, donGia
//			sql = "SELECT m.maMonAn, m.tenMonAn, ct.soLuonq, m.giaBan, (ct.soLuonq * m.giaBan) as thanhTien, N'Đã đặt trước' as trangThaiPhucVu, 0 as id_cthd "
//					+ "FROM ChiTietDonDatMon ct " + "JOIN MonAn m ON ct.maMonAn = m.maMonAn "
//					+ "JOIN DonDatMon d ON ct.maDon = d.maDonDat " + // Kiểm tra lại cột join d.maDonDat hay d.maDon
//					"WHERE d.maBan = ? AND d.trangThai = N'Chờ khách'";
//		}
//
//		try {
//			java.sql.Connection con = connectDatabase.ConnectDB.getInstance().getConnection();
...
//				m.soLuong = rs.getInt(3); // Dùng số thứ tự cột cho chắc vì chữ 'soLuonq' dễ gõ sai
//				m.giaBan = rs.getLong("giaBan");
...
//	}
	...
	public List<MonAnModel> getMonAnTheoBan(String maBan, String trangThai) {
		...
		if (trangThai.equalsIgnoreCase("Có khách")) {
			...
			// Lấy từ Phiếu đặt nếu bàn màu vàng
			sql = "SELECT m.maMonAn, m.tenMonAn, c.soLuong, m.giaBan, (c.soLuong * m.giaBan) as thanhTien, N'Đã đặt trước' as trangThaiPhucVu, 0 as id_cthd "
					+ "FROM ChiTietPhieuDat c JOIN MonAn m ON c.maMonAn = m.maMonAn "
					+ "JOIN PhieuDatBan p ON c.maPhieu = p.maPhieu "
					+ "WHERE p.maBan = ? AND p.trangThai = N'Chờ khách'";
		}
